...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updatePassword = async (newPassword: string) => {
    try {
      console.log('[PasswordConfirm] Updating password...');
      const { error } = await supabase.auth.updateUser({
        password: newPassword,
      });
      if (error) {
        console.error('[PasswordConfirm] Update failed:', error.message);
        throw error;
      }
      console.log('[PasswordConfirm] Password updated successfully');
    } catch (err) {
      console.error('[PasswordConfirm] Unexpected error:', err);
      throw err;
    }
  };
